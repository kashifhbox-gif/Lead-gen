...
import { Activity, ExternalLink, ArrowLeft, ThumbsUp, MessageSquare, BrainCircuit, CheckCircle2, XCircle, Loader2, Calendar, Mail, Edit2, Save, X, Clock } from 'lucide-react';
...
export default function LeadDetailsPage() {
  ...
  const isAutomationFinished = (lead?.jobId?.status === 'COMPLETED' || lead?.jobId?.status === 'FAILED') && (lead?.jobId?.emailEnrichmentStatus === 'COMPLETED' || lead?.jobId?.emailEnrichmentStatus === 'FAILED' || !lead?.jobId?.totalEnrichmentTarget);
  const isAutomationActive = lead?.jobId ? !isAutomationFinished : false;
    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-3 items-center">
          {lead?.profileUrl && (
            <>
              {(!lead.firstPersonalEmail || !(lead.phones && lead.phones.length > 0)) && !isAutomationActive && (
                <button
              ...
              {(!lead.firstPersonalEmail || !(lead.phones && lead.phones.length > 0)) && isAutomationActive && (
                <span className="text-neutral-500 text-sm font-medium flex items-center gap-1.5 px-2" title="Waiting for automated enrichment batch">
                  <Clock className="w-4 h-4" /> Queued for Auto-Enrichment
                </span>
              )}
            </>
          )}
